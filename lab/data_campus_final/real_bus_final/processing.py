...
class make_data:
    ...
    def make_coordinate(bus_speed_mean_data, bus_speed_route_data):
        ...
        bus_speed_mean_coordinate_data.rename(columns=lambda col: col.replace(':00', '시'), inplace=True) # ':00' 을 '시'로 바꿔주는 코드
    ...
    def speed_final_data(coordinate_data, time):
        coordinate_data_time = coordinate_data.copy()
        time_column = f'{time}시'
        
        coordinate_data_time['speed_range'] = 'medium'
        coordinate_data_time.loc[coordinate_data_time[time_column] >= 20, 'speed_range'] = 'fast'
        coordinate_data_time.loc[coordinate_data_time[time_column] < 10, 'speed_range'] = 'slow'
        
        return coordinate_data_time
    
    # def speed_final_data(coordinate_data, time):
    #     coordinate_data_time = coordinate_data[['SECTION','LineString',f'{time}시','STATION_1_BUSSTOP_TP','STATION_2_BUSSTOP_TP']]
    #     coordinate_data_time.loc[coordinate_data_time[f'{time}시'] >= 20, 'speed_range'] = 'fast'
    #     coordinate_data_time.loc[(coordinate_data_time[f'{time}시'] >= 10) & (coordinate_data_time[f'{time}시'] < 20), 'speed_range'] = 'medium'
    #     coordinate_data_time.loc[coordinate_data_time[f'{time}시'] < 10, 'speed_range'] = 'slow'
    #     return coordinate_data_time
    
    # def make_up_down(exdata):
    #     starting_point_1 = exdata[exdata['STATION_1_BUSSTOP_TP'] == '2'].index
    #     starting_point_2 = exdata[exdata['STATION_2_BUSSTOP_TP'] == '2'].index

    #     if len(starting_point_1) > 0:
    #         starting_point = starting_point_1[0]
    #     else:
    #         starting_point = starting_point_2[0]

    #     # UP/DOWN 구분
    #     exdata_up = exdata[:starting_point + 1]
    #     exdata_down = exdata[starting_point + 1:]
    #     return exdata_up, exdata_down
    
    def make_up_down(exdata):
        STARTING_POINT = int(len(exdata)/2)+1 # 기점
        # UP/DOWN 구분
        exdata_up = exdata[:STARTING_POINT]
        exdata_down = exdata[STARTING_POINT:]
        return exdata_up, exdata_down
